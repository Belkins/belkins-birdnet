...
| D17 | Voucher Export ("Accession Passport") | **DECIDED-YES (concept)** | 2026-08-01 | Arsenal innovation winner: package a species' pinned first-confident clip + spectrogram + accession no. + sha256 hashes into a portable verifiable specimen card — chains the accession ledger, purge protection, and R2 backup; fulfills D6's sanctioned-sharing channel. Slice (`voucher_export.py`, ≤1 day) scheduled Sprint 2–3. Hard rules: never marketed as "proof" (AI-only single-mic ID), and exports must not embed precise station coordinates by default |
